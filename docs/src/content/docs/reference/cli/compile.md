...
| `--clean` | Remove orphaned AGENTS.md files no longer produced by the current primitive set. Nested Git repositories and linked worktrees below the cleanup root are excluded; to clean one, make that checkout the cleanup root, usually by running `apm compile --clean` there. For `--target claude`, also removes a stale APM-generated CLAUDE.md once instructions live in `.claude/rules/`. Hand-authored CLAUDE.md files (files without the `<!-- Generated by APM CLI -->` marker) are never deleted. |
